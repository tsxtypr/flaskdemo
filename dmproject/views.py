...
from models import *
from sdk.get_date import MyDate
from settings import STATIC_PATH
from sdk.page import Pager
...
def leave_list():
    ...
    if request.method == 'POST':
        ...
        if len(data.get('username'))!=0 and len(data.get('starttime'))!=0 and len(data.get('endtime'))!=0 and len(data.get('description'))!=0 and len(data.get('phone'))!=0:
            leave = Leave()
            leave.leave_id = int(user_id)
            leave.leave_name = data.get('username')
            leave.leave_type = data.get('type')
            if data.get('starttime'):
                leave.leave_start = datetime.strptime(data.get('starttime'),'%Y-%m-%d')
            if data.get('endtime'):
                leave.leave_end = datetime.strptime(data.get('endtime'),'%Y-%m-%d')
            ...
            leave.save()
        return redirect('/leave_all_list/'+str(pager.num_pages)+'/')
    return render_template('leave_list.html')


## 当前登录用户的所有请假条
@app.route('/leave_all_list/<int:page>/')
def leave_all_list(page):
    user_id = request.cookies.get('user_id')
    leave = Leave.query.filter(Leave.leave_id == user_id).order_by(Leave.leave_status).all()
    pager = Pager(leave,10)
    page_obj = pager.page_data(page)
    ...
    if end>pager.num_pages:
        ...
        start = end-5
    page_range = list(pager.page_range)[start:end]
...
## 实现请假条撤销功能
@app.route('/leave_back/',methods=['post','get'])
@loginValid
def leave_back():
    result = {
        'code':1000,
